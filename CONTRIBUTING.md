...
| project | purpose |
|---|---|
| `Launcher` | GUI launcher (WPF, Windows-only) |
...
`Launcher` and `Launcher.Cli` are two front ends over the same `Launcher.Core` pipeline —
neither duplicates the download/verification/injector logic. Keep it that way: a fix
belongs in `Launcher.Core` unless it's genuinely specific to one front end's UI.

## Requirements

.NET SDK 9.0+. `Launcher` (the WPF GUI) only builds on Windows; the rest of the solution is
cross-platform.
...
dotnet build Launcher/Valheim-Online_Launcher.csproj   # Windows only (WPF)
...
A release build of the GUI, as a single distributable file:
...
Both suites run in CI via GitHub Actions on every push/PR. See each project's own README
...
- **`Loc`** — localization. To add a 10th language: add its code to `Loc.Supported` and its
